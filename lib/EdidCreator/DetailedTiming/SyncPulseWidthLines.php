<?php
/**
 * Contains SyncPulseWidthLines class.
 *
 * PHP version 5.3
 *
 * LICENSE:
...
/**
 * Class SyncPulseWidthLines
 *
 * @package EdidCreator\DetailedTiming
 */
class SyncPulseWidthLines extends AbstractDetailedTimingComponent
{
    /**
     ...
     * @return self
     */
    public function setSyncPulseWidthLines($value)
    {
        $fieldLength = 6;
        $value = $this->convertValueToBitString($value, $fieldLength);
        $msb = substr($value, 0, 2);
        $offset = array($this->byteOffset + 11, 0);
        $this->edid->setBitField($msb, $offset);
        $value = substr($value, -4);
        $offset = array($this->byteOffset + 10, 0);
     ...
     * @return string
     */
    public function getSyncPulseWidthLines()
    {
        $offset = array($this->byteOffset + 11, 0);
